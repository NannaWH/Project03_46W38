...
- ** inputs **
    ...
    - *** LocationX.csv-Location.csv: *** historical power and weather data for location X (1-4)
...
- ** src **
    ...
    - *** C_persistence_model.py: *** the file establishes a functions that develop a persistence predicting model to predict power output 1 hour and 6 hours into the future.
...
The coloured boxes (green and orange) are the actual forecasting models. The cleaned and split data from the white boxes goes into these models to be trained and tested. The green boxes are forecasting models defined by functions. The persistence model is a simple and naive forecasting method for which it is assumed that the current power production would be the power production in e.g., one or six hours. The random forest model is a ML model that uses decision trees to find patterns in the data and forecast the power production. 

The orange box is a forecasting model defined by a class. In this class we have defined a model for forecasting the power output using a neural network ML model that learns patterns from interconnected neurons. In the class the function variables are defined, and a function is built to first drop irrelevant variables and second define and train the ML model. For the neural network model, both Keras and the MLPregressor from scikit-learn can be used to train the model. The class is defined in the src/E_neural_network_model.py file.

They grey boxes indicate graph plotting functions. These makes graph plots of either the raw data or the predicted outputs from the forecasting models. The three boxes connected to the blue raw data box are functions plotting the raw data. The graphs produced by these functions have been used in the preliminary phases of the model development to get a sense of the relationships between power output and explanatory variables. Additionally, there are functions plotting the predicted power output against the actual power output. These plots can be used to get a visual sense of the models’ performances. Lastly, the pink box indicates a function that calculates the error measures of the forecasting models. These error measures are a powerful way to compare the performance of the different models.

<img width="720" height="405" alt="Image" src="https://github.com/user-attachments/assets/ba03255c-f3be-47d3-9eec-72539c0a2b11" />
